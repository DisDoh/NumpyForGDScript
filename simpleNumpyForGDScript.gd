...
# Function to compare two matrices using a tolerance (to account for floating point differences)
...
func test_zeros():
	...
	print("zeros() took: ", end_time - start_time, " ms")
	assert(matrices_equal(mat, [[0, 0, 0], [0, 0, 0]]), "Error in zeros()")
...
func test_matmul():
	...
	var end_time = Time.get_ticks_msec()
	print("matmul() took: ", end_time - start_time, " ms")
	assert(matrices_equal(result, expected), "Error in matmul()")
...
func test_transpose():
	...
	var end_time = Time.get_ticks_msec()
	print("transpose() took: ", end_time - start_time, " ms")
	assert(matrices_equal(result, expected), "Error in transpose()")

func run_tests():
	print("Launching all tests with time tracking...")
	test_zeros()
	test_matmul()
	test_transpose()
